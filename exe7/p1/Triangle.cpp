 ...
 * cpp lab - Meir Litman                                   *
 * exercise 7                                              *
 * class implement Triangle based on Shape abstract class  *
************************************************************/
...
public:
    ...
    double area() const override { // return area of Triangle using Heron's formula
        double s =
                (points[0].distance(points[1]) + points[1].distance(points[2]) + points[2].distance(points[0])) / 2.0;
        return sqrt(s * (s - points[0].distance(points[1])) * (s - points[1].distance(points[2])) *
                    (s - points[0].distance(points[2])));
    }
};
